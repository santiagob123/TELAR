...
type Message = { from: string; text: string; isFromAI?: boolean }
  ...
  useEffect(() => {
    async function setup() {
      try {
        ...
        if (!initResponse.ok || typeof initData.businessId !== 'string') {
          throw new Error(initData?.error || 'No se pudo preparar el contexto de TELAR AI.')
        }
  ...
  async function sendText(text: string) {
    ...
    try {
      ...
      const res = await fetch('/api/ai/message', {
        ...
        body: JSON.stringify({ businessId, from: '+demo', text })
      })
      if (res.status === 401) {
        await router.replace('/ai/login')
        return
      ...
      setMessages(prev => [...prev, { from: 'agent', text: aiText, isFromAI: true }])
  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        <button onClick={() => sendText('¿Cuál es el horario?')} className="rounded-full border border-[#cbd9d6] bg-white px-4 py-2 text-sm font-semibold text-[#075c59] transition hover:bg-[#e9f2ef]">Consultar horario</button>
        <button onClick={() => sendText('¿Cuál es el contacto?')} className="rounded-full border border-[#cbd9d6] bg-white px-4 py-2 text-sm font-semibold text-[#075c59] transition hover:bg-[#e9f2ef]">Consultar contacto</button>
        <button onClick={() => sendText('¿Cuál es el precio?')} className="rounded-full border border-[#cbd9d6] bg-white px-4 py-2 text-sm font-semibold text-[#075c59] transition hover:bg-[#e9f2ef]">Consultar precio</button>
      </div>
        ...
        <div className="flex-1 overflow-auto" ref={listRef}>
          {messages.length === 0 && (
            <div className="flex h-full items-center justify-center px-6 text-center">
              <div><p className="text-lg font-semibold">Empieza una conversación</p><p className="muted mt-2 text-sm">Elige una pregunta de ejemplo o escribe un mensaje.</p></div>
            </div>
          )}
          {messages.map((m, i) => (
          ...
          {isLoading && (
            <div className="my-2 mr-auto max-w-[75%]">
